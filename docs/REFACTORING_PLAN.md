...
| 기능 | 라인 | 사용 빈도 | 판단 |
|------|------|----------|------|
| `desk_delete_legacy()` | 379-414 | LEGACY_CALL 관련 | ❓ 리뷰 |
| `desk_reset_dedup()` | 334-376 | dedup 초기화 | ❓ 리뷰 |
...
```
desk/src/routes/
├── desk.py                 # 기본 뷰 + 기사 CRUD (~400줄)
├── desk_publish.py         # 발행 관련 (~300줄)
└── desk_schedule.py        # 스케줄 관리 (~200줄)
```
